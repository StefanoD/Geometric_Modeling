...
{	// initialize all elements with zero
    for (unsigned i=0; i<SIZE; i++) m_atData[i] = T(0);
}
...
{
    for (unsigned i=0; i<SIZE; i++) m_atData[i] = atData[i];
}
...
{
    for (unsigned i=0; i<SIZE; i++) m_atData[i] = vec(i);
}
...
{
    for (unsigned i=0; i<SIZE; i++) atData[i] = m_atData[i];
}
...
template <class T, unsigned SIZE>
void CVector<T,SIZE>::swap(unsigned i, unsigned j) // swaps i-th and j-th coordinates
{
...
{
    T d= 0;
    for (unsigned i=0; i < SIZE; i++) d += m_atData[i] * m_atData[i];
    return d;
}
...
void CVector<T,SIZE>::normalize ()
{
    T d = norm();
    // Normalize vector
    for (unsigned i=0; i<SIZE; i++) m_atData[i] = m_atData[i] / d;
}
...
{
    for (unsigned i=0; i<SIZE; i++) m_atData[i] *= tScale;
}
